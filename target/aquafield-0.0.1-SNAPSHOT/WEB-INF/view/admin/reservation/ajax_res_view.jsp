	...
	<div class="inner">
				...
				<div class="tb_type2 mb20">
					<table>
						<tbody>
							<tr>
								<th>예약번호</th>
								<td colspan="3">${vo.RESERVE_UID }</td>
							</tr>
							<tr>
								<th>방문일</th>
							...
							<tr>
								<th>회원명</th>
								<td>
									<!-- 20160829 김민주 추후 권한 따라 다르게 출력할것 회원명, 휴대폰번호 -->
									${vo.MEM_NM_MASK}<%-- ${vo.MEM_NM} --%>( ${vo.MEM_ID} )
								</td>
								<th>결제방법<!-- 20160830 김민주 현재 코드값으로 되어있음 추후 변경 해야함 --></th>
							...
							<tr>
								<th>휴대폰번호</th>
								<td>${vo.MEM_MOBILE_MASK}<%-- ${vo.MEM_MOBILE} --%></td>
								<th>결제금액</th>
		...
		<footer>
			<div class="bot_btns">
				...
				<a href="#" onclick="goDetail('${vo.RESERVE_UID}'); return false;" class="btn_pack btn_mo gray"><img src="/common//admin/images/common/ico_edit.png">수정</a>
			</div>
		</footer>
	</div>
	<script type="text/javascript">
		...
		$( document ).ready(function() {
		    ...
		    var $event = $(".event");
		   	var $water = $(".water");
		   	var $spa = $(".spa");
		    var cata_code = "${vo.ORDER_NM}";
		    
		    if($nomal.length != 0 && cata_code == "입장상품" ){   	$nomal.eq(0).text("입장상품"); 	   }
			if($nomal.length != 0 && cata_code == "패키지" ){		$nomal.eq(0).text("패키지"); 		}
		...
		function resCompleteUse(resIdx){
			...
			if( stat == 'ING' || stat == 'NOUSE' ){
				$.ajax({
					method: "POST",
					url: "/admin/reservation/resCompleteUse.af",
					dataType: "JSON",
