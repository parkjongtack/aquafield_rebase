...
@Controller
public class NaverLoginController extends GenericController {
	...
	@RequestMapping(value = "/member/naverlogin.af")
    public String kakaologinController(@RequestParam Map param, Model model, HttpSession session, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for(int i = 0; i < 5; i++) {
		    ...
		    dValue = dValue + (int)(Math.random() * 10);
		...
		*/
		...
		parameter.put("home_addr1", "naver");
		parameter.put("home_addr2", "naver");
		parameter.put("company_phone_num", "naver");
		parameter.put("company_nm", "naver");
		parameter.put("company_addr1", "naver");
		parameter.put("company_addr2", "naver");
		parameter.put("memory_day", "naver");
		parameter.put("terms_at", "Y");
		parameter.put("received_info_at", "N");
		parameter.put("mem_rating", "naver");
		...
		//if(niceChk > 0) { 	
			...
			if(idChk == 0) {
				...
				params.put("recipient_num", DecoderUtil.decode(param, "phone_number")); // �닔�떊踰덊샇
				...
				*/
				...
				parameter1.put("mem_pw", "naver");	//�븫�샇�솕 �옉�뾽�빐�빞�븿 
