...
public class AdminCustomerController extends GenericController {
	...
	@RequestMapping("/admin/customer/index.af")
	public String index(@RequestParam Map param, Model model, HttpSession session, HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
		...
		List codeMEM_TYPE = super.getCommonCodes("MEM_TYPE");		
		//답변등록 결과
		...
		model.addAttribute("codeMEM_TYPE", codeMEM_TYPE);		
	 ...
	 * 고객문의 상세
	 */
	@RequestMapping("/admin/customer/contact_view.af")
	public String contact_view(@RequestParam Map param, Model model, HttpSession session, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		logParam.put("data_num", vo.get("WRITER").toString() + "회원님의 고객문의를 열람 하셨습니다.");
		...
		return "/admin/customer/contact_view";
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping("/admin/customer/contact_update.af")
	public String contact_update(@RequestParam Map param, Model model, HttpSession session, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String data_url = httpName + "/secu_admaf/admin/customer/contact_view.af?ask_uid="+data_num;
		String etcContent = "수정";
		String etc = param.get("ask_nm").toString()+"("+ param.get("ask_phone").toString() +") " + etcContent ;
		
		param.put("ask_uid", param.get("ask_uid").toString());
		Map vo = customerService.customerDetail(param);
		...
		logParam.put("data_num", vo.get("WRITER").toString() + "회원님의 고객문의에 답변을 등록하셨습니다.");
...
//		}
		...
		/* 20180517 syw : 고객문의 답변 등록시 오류로 주석처리함.*/
		...
		System.out.println("2019. 3. 20. Author:ldg contact_update start ==== ");
		try {
			...
			System.out.println("2019. 3. 20. Author:ldg contact_update end booleanresult ==== " + booleanresult);
			if(!booleanresult){
		...
		return "redirect:/secu_admaf/admin/customer/index.af?result=1";
	}
