...
public class AdminDeskAdminLoginController extends GenericController {
	...
	@RequestMapping(value="/admdesk/login.af", method = RequestMethod.GET)
	public String login(@RequestParam Map param, ModelMap model, HttpSession session, HttpServletRequest request, HttpServletResponse response){
		...
		return "/admdesk/login";
	}
	   
	@RequestMapping(value="/admdesk/login.af", method = RequestMethod.POST)
	public void loginAction(@RequestParam Map param, ModelMap model, HttpSession session, HttpServletRequest request, HttpServletResponse response) throws Exception {		
		...
		try{
			...
			if(param.get("point") == null || param.get("point").toString().equals("") ) param.put("point","sldfslfaslifdalsfsldf");
			...
			sqlMap.put("point",param.get("point").toString() );
			sqlMap.put("hpnum",param.get("hpnum").toString() );
			sqlMap.put("certnum",param.get("certnum").toString() );
			
			loginUser = adminDeskAdminLoginService.deskAdminLogin(sqlMap);
			if(loginUser == null){
				session.invalidate();
				logger.debug("로그인 실패 ADMIN_ID : " + param.get("login_id"));
				Util.htmlPrint("{\"result\":false,\"msg\":\"아이디 또는 비밀번호, 지점, 인증번호를 확인하세요!\"}", response);
				return;
			}
			else{
				...
				session.setAttribute(CommonConstant.session.SESSION_KEY_DESK_ADMIN, user);
				
				//-- 로그인 정보
				sqlMap.put("login_digit",loginDigit);
				
				//-- 로그인 정보수정
				adminDeskAdminLoginService.updateDeskAdminLoginDigit(sqlMap);
	...
	@RequestMapping(value="/admdesk/login_sms.af", method = RequestMethod.POST)
	public void loginSms(@RequestParam Map param, ModelMap model, HttpSession session, HttpServletRequest request, HttpServletResponse response) throws Throwable {	
		...
		Map<String,String> adminUser = adminDeskAdminLoginService.deskAdminLogin(sqlMap);
		//-- 관리자 정보가 있다.
		if(adminUser != null){
			//-- 문자메세지 발송
			sqlMap = new HashMap<String,String>();
			...
			//-- 비교할 문자를 관리자 정보에 넣는다.
			adminDeskAdminLoginService.updateDeskAdminCertnum(sqlMap);
			...
			try{
				if(smsService.sendSms(sqlMap) ){
					Util.htmlPrint("{\"result\":true,\"msg\":\"인증번호를 전송하였습니다.\"}", response);
	...
	@RequestMapping(value="/admdesk/logout.af")
	public String logout(@RequestParam Map param, ModelMap model, HttpSession session, HttpServletRequest request, HttpServletResponse response) throws Throwable {
		String reqUrl = request.getRequestURI().toString();
		String modelUrl = "";
		modelUrl = "redirect:/secu_admaf/admdesk/login.af";
		session.invalidate();
		return modelUrl;
	}
