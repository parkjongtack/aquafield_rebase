...
<!-- <link rel="stylesheet" type="text/css" href="/admaf/css/datepicker.css"> -->
<!--[if lt IE 9]>
	<script src="/admaf/js/html5.js"></script>
<![endif]-->
<!--[if lt IE 8]>
...
<script type="text/javascript" src="/admaf/js/common2.js"></script>
	</head>
<body>		
		...
		<div class="loginBox">
			...
			<div class="loginBody">
				...
				<form name="form" id="form" method="post" action="login.af" onsubmit="return false;">
					...
					<div class="inputBox">
						<ul>
							<li><label>ID (E-MAIL)</label><input type="text" name="login_id" id="login_id" class="inputST1" value="bko@oktomato.net" placeholder="ex) abc@daum.net" /></li>
							<li><label>PASSWORD</label><input type="password" name="login_pw" id="login_pw" class="inputST1" value="1111qqqq" /></li>
							<li>
								<label>휴대폰번호</label><input type="text" name="hpnum" id="hpnum" class="inputST1 input_cert" value="010-3896-9290" />
								<div class="btnBox"><button type="button" class="btn_cert btn_pack btn_mo" id="btnCert" onclick="chkCert();">인증</button></div>
							</li>
							<li><label>인증번호</label><input type="number" name="certnum" id="certnum" class="inputST1" value="236637" placeholder="숫자만 입력해 주세요."/></li>
						</ul>
					</div><!-- inputBox -->
					<div class="btnBox">
						<div class="save_id">
...
function chkForm(){
	...
	if($.trim(document.form.certnum.value) == ""){ 
		alert("인증번호를 입력하세요!"); 
		document.form.certnum.focus(); 
		return false; 
	}
	...
	$.ajax({
		...
		,success: function(data){
			if(data.result == true){
				var returnurl = $("#returnurl").val() == "" ? "/secu_admaf/admin/index.af" : $("#returnurl").val();
				//var returnurl = $("#returnurl").val() == "" ? "/admin/reservation/index.af" : $("#returnurl").val();
