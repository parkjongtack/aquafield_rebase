...
<section id="pop_res_view_panel" class="pop_type1 bx_pop_type1">
	<div class="inner">
		...
		<article>
			<form action="/admin/reservation/res_edit.af" onsubmit="return false;">
				...
				<div class="tb_type5">
					<table>
						...
						<tbody>
							...
							<%-- 대여상품 시작 --%>
							<%-- <c:if test="${not empty vo.RENTAL1_ITEM && vo.RENTAL1_ITEM ne '-1' }">
								<tr>
							...
							<c:if test="${not empty vo.RENTAL2_ITEM && vo.RENTAL2_ITEM ne '-1' }">
								<tr>
									<td class="rental"><!-- 대여상품 -->&nbsp;</td>
							...
							<c:if test="${not empty vo.RENTAL3_ITEM && vo.RENTAL3_ITEM ne '-1' }">
								<tr>
									<td class="rental"><!-- 대여상품 -->&nbsp;</td>
							...
							이벤트상품 시작
							<c:if test="${not empty vo.EVENT1_ITEM && vo.EVENT1_ITEM ne '-1' }">
								<tr>
							...
							<c:if test="${not empty vo.EVENT2_ITEM && vo.EVENT2_ITEM ne '-1' }">
								<tr>
									<td class="event"><!-- 이벤트상품 -->&nbsp;</td>
							...
							<c:if test="${not empty vo.EVENT3_ITEM && vo.EVENT3_ITEM ne '-1' }">
								<tr>
									<td class="event"><!-- 이벤트상품 -->&nbsp;</td>
		...
		<footer>
			<div class="bot_btns">
				...
				<c:if test="${vo.RESERVE_STATE ne 'NOPMT' }">
					<a href="javascript:;" onclick="goDetail('${vo.ORDER_NUM}'); return false;" class="btn_pack btn_mo gray"><img src="/admaf/images/common/ico_edit.png">수정</a>
				</c:if>
			</div>
		</footer>
	</div>
...
//		    if($water.length != 0){		$water.eq(0).text("워터파크");		}
		...
		function resCompleteUse(resIdx){
			...
			if( stat == 'ING' || stat == 'NOUSE' ){
				$.ajax({
					method: "POST",
					url: "/secu_admaf/admin/reservation/resCompleteUse.af",
					dataType: "JSON",
					data : { reserve_uid : resIdx, order_num : "${vo.ORDER_NUM }", order_nm : "${vo.ORDER_NM }" },
