/*예약결제 관련 팝업창*/
var cntVal = 0;
var reservationPop2Fn2 = function(params){
	params = params || {};
    	...
    	$.each(_this.resData['item'], function(i, v) {
    		 ...
    		 if(v.code.indexOf('spa')!=-1 || v.code.indexOf('water')!=-1 || v.code.indexOf('complex')!=-1){
    		 	if(v.price > 0) {
    		 		...
    		 		$('.item_list table').append('<tr><td>'+v.title+v.age+'</td><td>X '+count+'명</td><td>= '+priceCommas(v.price)+'원</td><td><span class="del" onclick="reservationPop2.delResItem(\''+v.code+'\');">x</span></td></tr>')
    		 ...
    		 if(v.code.indexOf('rantal')!=-1){
    		 	if(v.price > 0) {
    		 		...
    		 		$('.item_list table').append('<tr><td>'+v.title+'</td><td>X '+count+'명</td><td>= '+priceCommas(v.price)+'원</td><td><span class="del" onclick="reservationPop2.delResItem(\''+v.code+'\');">x</span></td></tr>')
    		 		_this.resData.payment += v.price;
    		 		rtProdUid +=v.uid + ",";
    		 		rtProdCnt += count;
    		 	}
...
var reservationFn2 = {
	...
	date : function(me){
        if(reservationPop2.resData.payment > 0) {
            if(confirm('방문일을 변경하시면 이전 예약상품이 초기화 됩니다. 계속하시겠습니까?')) {
                reservationPop2.resData = { 'pointCode':reservationPop2.resData.pointCode, 'pointNm':reservationPop2.resData.pointNm, 'date':reservationPop2.resData.date,'type':reservationPop2.resData.type,'item':{},'payment':0, 'dpayment':0 };
                reservationPop2.setResList();
            }else{
                return false;
            }
        }
        reservationPop2.addSubSect({url : '/reserve/date.af',target:me, data :{ prodNum : $("#bx_multi_content > .on").find("input[name='prodNum']").val(), pointCode : reservationPop2.resData.pointCode} });
    },
    plant : function(me){
        /*if(reservationPop2.resCheck(0) == true) {
           reservationPop2.addSubSect({url : '/reserve/plant.af',target:me, data :{prodNum : $("#bx_multi_content > .on").find("input[name='prodNum']").val(),seldate : $(".date > a > .info > span").html(), pointCode : reservationPop2.resData.pointCode} }) ;
        }*/
    	
    	var data = { 'prodNum' : $("input[name='prodNum']").val() , 'seldate' : $("#date").val(), 'pointCode' : $("#pointCode").val()};
		var t = $('.'+me);
		$.ajax({
    ...
    package : function(me){
        /*if(reservationPop2.resCheck(0) == true) {
           reservationPop2.addSubSect({url : '/reserve/packagePlant.af',target:me, data :{prodNum : $("#bx_multi_content > .on").find("input[name='prodNum']").val(),seldate : $(".date > a > .info > span").html(), pointCode : reservationPop2.resData.pointCode} }) ;
        }*/
    	var data = { 'prodNum' : $("input[name='prodNum']").val() , 'seldate' : $("#date").val(), 'pointCode' : $("#pointCode").val()};
		var t = $('.'+me);
		$.ajax({
            url : '/reserve/packagePlant.af',
            data : data,
            dataType : 'html',
            type : 'GET',
            success : function(data){
    ...
    rantal : function(me){
        ...
        if(reservationPop2.resCheck() == true) {
          reservationPop2.addSubSect({url : '/reserve/rantal.af',target:me , data :{prodNum : '30000000' ,seldate : $(".date > a > .info > span").html(), spaVal : spaVal, wpVal : wpVal, cpVal : cpVal} });
        }
    },
    event : function(me){
        ...
        if(reservationPop2.resCheck() == true) {
            reservationPop2.addSubSect({url : '/reserve/event.af',target:me, data :{prodNum : '40000000' ,seldate : $(".date > a > .info > span").html(), spaVal : spaVal, wpVal : wpVal, cpVal : cpVal} });
        }
    },
    point : function(pointCode, pointNm){
        //if(reservationPop2.resData.date != '') {
//    	if(pointCode == 'POINT03'){
//    		alert('고양점은 서비스 준비중입니다.');
//            if(reservationPop2.resData.type == '입장상품'){
...
//    	}else{
    		if(confirm('지점을 변경하시면 이전 예약상품이 초기화 됩니다. 계속하시겠습니까?')) {
                reservationPop2.resData = { 'pointCode':pointCode, 'pointNm':pointNm, 'date':'','type':reservationPop2.resData.type,'item':{},'payment':0, 'dpayment':0 };
            }
//    	}
        //}else{
        //    reservationPop2.resData = { 'pointCode':pointCode, 'pointNm':pointNm, 'date':reservationPop2.resData.date,'type':reservationPop2.resData.type,'item':{},'payment':0, 'dpayment':0 };
        //}
        reservationPop2.setResList();
        if(reservationPop2.popBx.hasClass('show_right_sec')){
        	reservationPop2.popBx.removeClass('show_right_sec');
        }
    }
